    ...
    pub fn get_always_skills(&self) -> Vec<String> {
        ...
        for s in self.list_skills(true) {
            if let Some(meta) = self.get_skill_metadata(&s.name) {
                let nanobot_meta =
                    _parse_nanobot_metadata(meta.get("metadata").map(|s| s.as_str()).unwrap_or(""));
                if nanobot_meta
                    .get("always")
...
/// Parse nanobot metadata JSON from frontmatter value.
fn _parse_nanobot_metadata(raw: &str) -> HashMap<String, serde_json::Value> {
    if raw.is_empty() {
        return HashMap::new();
    }
